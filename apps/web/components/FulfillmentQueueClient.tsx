...
import { getFulfillmentQueue, updateDeliveryStatus, updateFulfillment, type Delivery } from '../lib/shopping-api';
...
export function FulfillmentQueueClient() {
  ...
  function refresh() { getFulfillmentQueue().then(setItems).catch((reason) => setError(reason instanceof Error ? reason.message : 'Unable to load fulfillment queue.')); }
  useEffect(() => { refresh(); }, []);
