          ...
          {cart.items.map((item) => (
            <article key={item.id} className="cart-item">
              ...
              <div className="cart-item__content">
                ...
                <div className="cart-item__footer"><PriceDisplay price={item.currentPrice} /><label>Quantity <input type="number" min="1" max="20" value={item.quantity} disabled={busyItem === item.id} onChange={(event) => update(item.id, Number(event.target.value))} /></label><button type="button" className="text-button" onClick={() => remove(item.id)} disabled={busyItem === item.id}>Remove</button></div>
              </div>
            </article>
          ))}
        </div>
